...
function loadTwitch() {
  ...
  RESPONSE.forEach(buildTwitchResults);
  
  function buildTwitchResults(channel) {
    if (channel.status == '404') {
      html += '<tr><td class="logo"><img class="logo" src="https://cambodianbeginnings.files.wordpress.com/2013/06/4ff19-blacksquare.jpg">'
      html += '<td class="text-center name">' + channel.name + '</td>'
      html += '<td class="text-center status">Not a valid account</td></tr>'
    } else {
      html += '<tr><td class="logo"><img class="logo" src="' + channel.logo + '">'
      html += '<td class="text-center name"><a href="' + channel._links.self + '">' + channel.name + '</a></td>'
      html += '<td class="text-center status">' + channel.status + '</td></tr></a>'
    };
  }
